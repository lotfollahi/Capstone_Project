...
def build_net(network, X, Y, num_classes, num_epochs, checkpoint_path, size_batch, Xval=None, Yval=None, dec_step=100,
    ...
    ###########################################################################
    network = conv_2d(network, 32, [3, 3], weights_init=nor, regularizer='L2', weight_decay=0.001)
    ...
    network = conv_2d(network, 64, [3, 3], weights_init=nor, regularizer='L2', weight_decay=0.001)
    network = batch_normalization(network)
    network = tflearn.activations.softplus(network)
    ...
    #network = local_response_normalization(network)
    '''  
    network = conv_2d(network, 128, [3, 3], weights_init=nor, regularizer='L2', weight_decay=0.001)
    network = batch_normalization(network)
    network = tflearn.activations.softplus(network)
    ...
    '''
    ###########################################################################
    network = fully_connected(network, 128, weights_init=nor, regularizer='L2', weight_decay=0.001)
    network = batch_normalization(network)
    network = tflearn.activations.softplus(network)
    ...
    ###########################################################################
    network = regression(network,optimizer=adadelta,loss='categorical_crossentropy')
