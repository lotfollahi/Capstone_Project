...
def build_net(network, X, Y, num_classes, num_epochs, checkpoint_path, size_batch, Xval=None, Yval=None, dec_step=100,
    ...
    ###########################################################################
    network = conv_2d(network, 32, [3, 3], weights_init=nor, regularizer='L2')
    network = batch_normalization(network)
    network = tflearn.activations.softplus(network)
    network = max_pool_2d(network, [1, 2])
    network = local_response_normalization(network) 
    '''         
    network = conv_2d(network, 64, [1, 3], weights_init=nor, regularizer='L2')
    network = batch_normalization(network)
    network = tflearn.activations.softplus(network)
    ...
    adadelta = tflearn.optimizers.AdaDelta(learning_rate=0.1, rho=0.95, epsilon=1e-07)
